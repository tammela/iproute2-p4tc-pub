...
static int perm_str_to_u16(char *permissions_str)
{
	int permissions = 0;
	int i;

	if (strnlen(permissions_str, P4TC_PERM_MAX_BIT + 2) != P4TC_PERM_MAX_BIT + 1) {
		fprintf(stderr, "Invalid permissions string %s\n",
			permissions_str);
		return -1;
	}

	for (i = 0; i < P4TC_PERM_MAX_BIT + 1; i++) {
		if (permissions_str[i] != '-')
			permissions += (1 << (P4TC_PERM_MAX_BIT - i));
	}
			    ...
			    int *runt_params_offset)
{
	...
	ret = cjson_get_optional_string(table_cjson,
					...
					&permissions_str);
	if (!ret) {
		permissions = perm_str_to_u16(permissions_str);
		if (permissions < 0)
			return -1;
