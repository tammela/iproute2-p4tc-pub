/*
 * p4tc_template.c		P4 TC Template Management
 *
					      ...
					      struct rtattr *arg)
{
	...
	if (t->bitsz == 64) {
		value_u64 =
			rta_getattr_u64(tb[P4TC_EXT_PARAMS_VALUE_RAW]);
...
static int p4tc_extern_inst_print_param(FILE *f,
					struct p4tc_json_extern_insts_list *insts_list,
					struct rtattr *arg)
{
	struct p4tc_json_extern_insts_data *inst_data = NULL;
	...
	if (tb[P4TC_EXT_PARAMS_NAME]) {
		...
		if (insts_list)
			inst_data = p4tc_json_find_extern_data(insts_list,
							       name);
	}
	...
	if (tb[P4TC_EXT_PARAMS_BITSZ])
		bitsz = *((__u16 *) RTA_DATA(tb[P4TC_EXT_PARAMS_BITSZ]));

	if (inst_data) {
		if (inst_data)
			t = get_p4type_byarg(inst_data->type, &bitsz);
	} else if (tb[P4TC_EXT_PARAMS_TYPE]) {
		__u32 contain_id;

		contain_id =
			*((__u32 *) RTA_DATA(tb[P4TC_EXT_PARAMS_TYPE]));
	...
	if (bitsz && !inst_data) {
		...
		if (t->flags & P4TC_T_TYPE_BIT)
			snprintf(buf, SPRINT_BSIZE - 1, "bit%u", bitsz);
		else if (t->flags & P4TC_T_TYPE_INT)
			snprintf(buf, SPRINT_BSIZE - 1, "int%u", bitsz);
		else
			strncpy(buf, t->name, SPRINT_BSIZE - 1);
	...
	if (tb[P4TC_EXT_PARAMS_VALUE]) {
		...
		p4tc_extern_inst_print_param_value(f, t,
				  ...
				  FILE *f)
{
	...
	for (i = 1; i < P4TC_MSGBATCH_SIZE + 1 && tb[i]; i++) {
		...
		p4tc_extern_inst_print_param(f, insts_list, tb[i]);
		close_json_object();
	}
...
static int print_extern_inst_template(struct nlmsghdr *n,
				      ...
				      __u32 extid, __u32 instid, FILE *f)
{
	...
	if (tb[P4TC_TMPL_EXT_INST_EXT_NAME]) {
		...
		fprintf(stderr, "extern name not specified\n");
		return -1;
	}
	...
	if (tb[P4TC_TMPL_EXT_INST_NAME]) {
		...
		fprintf(stderr, "extern instance name not specified\n");
		return -1;
	}

	insts_list = p4tc_json_find_extern_inst(pipe, extname, instname);
	if (!insts_list)
		return -1;
	...
	if (tb[P4TC_TMPL_EXT_INST_CONTROL_PARAMS]) {
		...
		p4tc_extern_inst_print_params(tb[P4TC_TMPL_EXT_INST_CONTROL_PARAMS],
					      insts_list, f);
	...
	if (tb[P4TC_TMPL_EXT_INST_CONSTR_PARAMS]) {
		...
		p4tc_extern_inst_print_params(tb[P4TC_TMPL_EXT_INST_CONSTR_PARAMS],
					      insts_list, f);
		close_json_array(PRINT_JSON, NULL);
	}
...
static int p4tc_print_extern_inst_template_flush(struct nlmsghdr *n,
						 struct rtattr *cnt_attr,
						 FILE *f)
{
	const __u32 *cnt = RTA_DATA(cnt_attr);

	print_uint(PRINT_ANY, "extinstcount", "    extern inst flush count %u",
		   *cnt);
	print_nl();
				   ...
				   __u32 extid, FILE *f)
{
	...
	if (tb[P4TC_TMPL_EXT_PERMISSIONS]) {
		__u16 *permissions = RTA_DATA(tb[P4TC_TMPL_EXT_PERMISSIONS]);
...
static int p4tc_print_extern_template_flush(struct nlmsghdr *n,
					    struct rtattr *cnt_attr,
					    FILE *f)
{
	const __u32 *cnt = RTA_DATA(cnt_attr);

	print_uint(PRINT_ANY, "extcount", "    extern flush count %u", *cnt);
	print_nl();
...
static int p4tc_print_table_timer_profile(struct rtattr *arg, FILE *f)
{
	...
	if (tb[P4TC_TIMER_PROFILE_AGING]) {
		print_lluint(PRINT_ANY, "profile_aging",
			     "        profile aging %u\n",
                             rta_getattr_u64(tb[P4TC_TIMER_PROFILE_AGING]));
	}
			    ...
			    struct rtattr *arg, __u32 tbl_id, FILE *f)
{
	...
	if (tb[P4TC_TABLE_EXTERNS]) {
		struct rtattr *tb_exts[P4TC_TABLE_EXTERN_MAX + 1];

		parse_rtattr_nested(tb_exts, P4TC_TABLE_EXTERN_MAX,
				    tb[P4TC_TABLE_EXTERNS]);

		if (tb_exts[P4TC_TABLE_EXTERN_COUNTER]) {
			const char *counter_inst_path =
				RTA_DATA(tb_exts[P4TC_TABLE_EXTERN_COUNTER]);

			print_string(PRINT_ANY, "counter_inst_path",
				     "    counter inst path %s\n",
				     counter_inst_path);
		}

		if (tb_exts[P4TC_TABLE_EXTERN_METER]) {
			const char *meter_inst_path =
				RTA_DATA(tb_exts[P4TC_TABLE_EXTERN_METER]);

			print_string(PRINT_ANY, "meter_inst_path",
				     "    meter inst path %s\n",
				     meter_inst_path);
		}
	}
				 ...
				 __u32 a_id, FILE *f)
{
	...
	if (tb[P4TC_ACT_NAME]) {
		...
		print_string(PRINT_ANY, "aname", "    action name %s\n", name);
			  ...
			  struct p4tcmsg *t, FILE *f)
{
	...
	case P4TC_OBJ_EXT:
		ids = RTA_DATA(tb[P4TC_PATH]);
		if (cmd == RTM_DELP4TEMPLATE && (n->nlmsg_flags & NLM_F_ROOT)) {
			p4tc_print_extern_template_flush(n, tb[P4TC_COUNT], f);
		} else {
			if (tb[P4TC_PATH])
	...
	case P4TC_OBJ_EXT_INST:
		ids = RTA_DATA(tb[P4TC_PATH]);
		if (cmd == RTM_DELP4TEMPLATE && (n->nlmsg_flags & NLM_F_ROOT)) {
			p4tc_print_extern_inst_template_flush(n, tb[P4TC_COUNT],
							      f);
		} else {
			if (tb[P4TC_PATH])
				print_extern_inst_template(n, pipe,
							   tb[P4TC_PARAMS],
							   ids[0], ids[1], f);
			else
				print_extern_inst_template(n, pipe,
...
static void
__p4tc_extern_inst_add_param_val(const void *value, const __u32 sz,
				 struct nlmsghdr *n)
{
	...
	addattr_l(n, MAX_MSG, P4TC_EXT_PARAMS_VALUE_RAW, value, sz);
	addattr_nest_end(n, nest_val);
}

static int p4tc_extern_inst_add_param_val(struct p4tc_ext_param *param,
					  const char *value, struct nlmsghdr *n)
{
	...
	__p4tc_extern_inst_add_param_val(val.value, sz, n);
...
static int
p4tc_extern_add_key_value(struct p4tc_ext_param *param, const char *value,
			  struct nlmsghdr *n)
{
...
static void __p4tc_extern_inst_add_param(struct p4tc_ext_param *param,
					 struct nlmsghdr *n)
{
	addattrstrz(n, MAX_MSG, P4TC_EXT_PARAMS_NAME, param->name);
	if (param->id)
...
static int p4tc_extern_inst_add_param(struct p4tc_ext_param *param, char ***argv_p,
				      ...
				      struct nlmsghdr *n)
{
	...
	__p4tc_extern_inst_add_param(param, n);
				  ...
				  const void *value, struct nlmsghdr *n)
{
	...
	t = get_p4type_byid(param->type);
	sz = t->bitsz >> 3;
	__p4tc_extern_inst_add_param_val(value, sz, n);
				 ...
				 struct nlmsghdr *n)
{
	...
	if (p4tc_extern_inst_add_param(&param, &argv, &argc, add_raw_value,
				       n) < 0)
		return -1;
			     ...
			     char *p4tcpath[], int cmd, unsigned int *flags)
{
	...
	while (argc > 0) {
		...
		} else if (strcmp(*argv, "tc_acl") == 0) {
			NEXT_ARG();
			if (get_u16(&permissions, *argv, 0) < 0)
				return -1;
	...
	if (parsed_permissions)
		addattr16(n, MAX_MSG, P4TC_TMPL_EXT_PERMISSIONS, permissions);
			    ...
			    char *p4tcpath[], int cmd, unsigned int *flags)
{
	...
	char *pname, *cbname, *tblname, *counter_path = NULL;
	char full_tblname[P4TC_TABLE_NAMSIZ] = {0};
	bool parsed_permissions = false;
	struct rtattr *tail = NULL;
	char *meter_path = NULL;
	char **argv = *argv_p;
	...
	while (argc > 0) {
		if (cmd == RTM_CREATEP4TEMPLATE ||
			...
			if (strcmp(*argv, "tblid") == 0) {
				...
				NEXT_ARG();
				counter_path = *argv;
			} else if (strcmp(*argv, "pna_direct_meter") == 0) {
				NEXT_ARG();
				meter_path = *argv;
	...
	if (counter_path || meter_path) {
		struct rtattr *nest;

		nest = addattr_nest(n, MAX_MSG,
				     P4TC_TABLE_EXTERNS | NLA_F_NESTED);
		if (counter_path)
			addattrstrz(n, MAX_MSG, P4TC_TABLE_EXTERN_COUNTER,
				    counter_path);

		if (meter_path)
			addattrstrz(n, MAX_MSG, P4TC_TABLE_EXTERN_METER,
				    meter_path);
