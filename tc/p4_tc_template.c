			     ...
			     char *p4tcpath[], int cmd, unsigned int *flags)
{
	...
	if (prealloc_acts)
		addattr32(n, MAX_MSG, P4TC_ACT_PREALLOC, prealloc_acts);
			       ...
			       char *p4tcpath[], int cmd, unsigned int *flags)
{
	...
	if (!hdrfield_id && !(hdrname && fieldname) &&
	    (cmd != RTM_CREATEP4TEMPLATE || cmd != RTM_UPDATEP4TEMPLATE)) {
		*flags |= NLM_F_ROOT;
	}
				  ...
				  int cmd, unsigned int *flags)
{
	...
	/* Always add count nest unless it's a dump */
	if (!count && !((*flags & NLM_F_ROOT) && cmd == RTM_GETP4TEMPLATE))
		count = addattr_nest(n, MAX_MSG, 1 | NLA_F_NESTED);
	...
	while (argc > 0) {
		if (cmd == RTM_CREATEP4TEMPLATE ||
			...
			if (strcmp(*argv, "tblid") == 0) {
				...
				NEXT_ARG();
				if (get_u64(&table.tbl_aging, *argv, 0) < 0)
					return -1;
				table.tbl_flags |= P4TC_TABLE_FLAGS_AGING;
