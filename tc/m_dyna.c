...
static int parse_act_path(char **p4tcpath, const char *actname,
			  char *actname_copy)
{
	int num_components;
	int ret;
...
int parse_dyna(int *argc_p, char ***argv_p, bool in_act, const char *actname,
	       struct nlmsghdr *n)
{
...
static int
parse_dyna_cb(const struct action_util *a, int *argc_p, char ***argv_p,
...
static int print_dyna_parm_value(FILE *f, const struct action_util *au,
				 ...
				 void *mask)
{
	...
	if (tb[P4TC_ACT_PARAMS_VALUE_RAW]) {
		...
		if (t->bitsz == 64) {
			value_u64 =
				rta_getattr_u64(tb[P4TC_ACT_PARAMS_VALUE_RAW]);
		...
		val.value = value;
		val.mask = mask ? RTA_DATA(mask) : NULL;
		if (t->print_p4t)
...
static int print_dyna_parm(FILE *f, const struct action_util *au,
			   struct rtattr *arg)
{
...
int print_dyna_parms(const struct action_util *au, struct rtattr *arg, FILE *f)
{
...
static int print_dyna(const struct action_util *au, FILE *f, struct rtattr *arg)
{
