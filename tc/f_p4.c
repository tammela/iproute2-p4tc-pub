...
static void get_elf_sec_name(char *elf_sec_name, char *prog_name)
{
	char trunc_elf_sec_name[MAX_BSS_SEC_PREFIX_LEN + 1] = {0};

	strncpy(trunc_elf_sec_name, prog_name, MAX_BSS_SEC_PREFIX_LEN);
...
static int
p4tc_check_bss_prog_map(int map_id, size_t p4tc_filter_fields_sz)
{
	...
	substr = strstr(info.name, ".bss");
	if (substr && info.max_entries == 1 && info.key_size == sizeof(__u32) &&
	    info.value_size == p4tc_filter_fields_sz) {
		return map_fd;
	}
...
static int
p4tc_get_bss_prog_maps(int fd, const int num_maps, size_t p4tc_filter_fields_sz)
{
	...
	for (i = 0; i < info.nr_map_ids; i++) {
		err = p4tc_check_bss_prog_map(map_ids[i],
...
static int p4tc_get_xdp_bss_map_fd(const char *pinned_prog,
				   size_t p4tc_filter_fields_sz)
{
	char elf_sec_name[MAX_BSS_SEC_LEN];
	int fd = bpf_obj_get(pinned_prog);
	struct bpf_prog_info info = {0};
	__u32 info_len = sizeof(info);
	int err, map_fd;

	if (fd < 0) {
		fprintf(stderr, "Unable to find pinned prog %s's fd\n",
			pinned_prog);
		return fd;
	}
	...
	if (err < 0) {
		fprintf(stderr, "Unable to get prog info by fd %d\n",
			fd);
		return err;
	}

	map_fd = p4tc_get_bss_prog_maps(fd, info.nr_map_ids,
					p4tc_filter_fields_sz);
	if (map_fd < 0)
		fprintf(stderr, "Unable to get map name %s\n", elf_sec_name);

	return map_fd;
}

static int p4tc_bss_map_update(int map_fd,
			       struct p4tc_filter_fields *p4tc_filter_fields)
{
	static const int zero = 0;
	int err;

	err = bpf_map_update_elem(map_fd, &zero, p4tc_filter_fields, BPF_ANY);
	if (err < 0) {
...
{
	char *basename_obj = basename(cfg->object);
	char elf_sec_name[MAX_BSS_SEC_LEN];
	...
	get_elf_sec_name(elf_sec_name, basename_obj);
	data_map = bpf_object__find_map_by_name(bpf_obj, elf_sec_name);
	if (!data_map || !bpf_map__is_internal(data_map)) {
		fprintf(stderr, "Failed to get data map %p for %s\n",
			data_map, elf_sec_name);
		err = -1;
		goto close_bpf_obj;
	}
	...
	err = p4tc_bss_map_update(map_fd, p4tc_filter_fields);
	if (err < 0)
		goto close_bpf_obj;
...
static int p4_parse_xdp_opt(int *argc_p, char ***argv_p,
			    struct p4tc_filter_fields *p4tc_filter_fields,
			    size_t p4tc_filter_fields_sz)
{
	char **argv = *argv_p;
	...
	map_fd = p4tc_get_xdp_bss_map_fd(*argv, p4tc_filter_fields_sz);
	if (map_fd < 0)
		return map_fd;

	ret = p4tc_bss_map_update(map_fd, p4tc_filter_fields);
...
static int p4_parse_opt(const struct filter_util *qu,
			struct tc_filter_fields *filter_fields,
			int argc, char **argv, struct nlmsghdr *n)
{
	...
	size_t p4tc_filter_fields_sz = 0;
	...
	while (argc > 0) {
		if (strcmp(*argv, "classid") == 0 ||
			...
			if (!runt_fields)
				goto cleanup;
		} else if (strcmp(*argv, "xdp_pinned") == 0) {
			if (!p4tc_filter_fields) {
				p4tc_filter_fields = p4tc_filter_fields_init(p,
									     filter_fields,
									     &p4tc_filter_fields_sz,
			...
			if (p4_parse_xdp_opt(&argc, &argv,
					     p4tc_filter_fields,
					     p4tc_filter_fields_sz) < 0)
				goto cleanup;
		} else if (strcmp(*argv, "help") == 0) {
			explain();
			goto cleanup;
...
static int p4_print_opt(const struct filter_util *qu, FILE *f,
