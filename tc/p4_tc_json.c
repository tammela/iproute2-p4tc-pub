...
static struct actions_list *_p4tc_find_act(struct table *tbl,
					   const char *act_name)
{
	struct actions_list *actions = tbl->actions;
				   ...
				   const char *act_name)
{
	...
	while (pipe->mat_tables) {
		//p4_tc_print_table(&mat_tables->table, stdout);
		action = _p4tc_find_act(&mat_tables->table, act_name);
		if (action)
			return action;
