...
struct profile *
p4tc_find_profile_by_aging(struct p4_tc_arch_json *a, __u64 aging_ms)
{
	struct profiles_list *profiles_list = a->profiles_list;

	while (profiles_list) {
		struct profile *profile = &profiles_list->profile;
...
static int json_parse_profile(cJSON *profile_cjson, struct profile *profile)
{
	char *name = NULL;
	int aging;
	int ret;
	...
	if (ret) {
		fprintf(stderr, "Failed to parse profile name:<%s> for:\n<%s>\n",
			JSON_PROFILE_NAME,
	...
	if (ret) {
		fprintf(stderr, "Failed to parse profile aging:<%s> for:\n<%s>\n",
			JSON_PROFILE_AGING,
			cJSON_Print(profile_cjson));
		return -1;
	}

	strncpy(profile->name, name, P4_TC_NAME_LEN - 1);
	profile->name[P4_TC_NAME_LEN - 1] = '\0';
...
static int json_parse_profiles(cJSON *root, struct p4_tc_arch_json *arch_info)
{
	struct profiles_list *profile_temp = NULL;
 ...
 * This function parses JSON describing tables for a P4 program
 */
struct p4_tc_arch_json *p4_tc_import_arch_json(const char *arch)
{
	struct p4_tc_arch_json *arch_info;
	char json_file_path[PATH_MAX];
	char *json_file_buffer;
	struct stat stat_b;
	size_t file_size;
