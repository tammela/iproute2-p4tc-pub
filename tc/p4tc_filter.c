...
#define PARAM_ACT_CBACTNAME_IDX 3
#define PARAM_ACT_PARAMNAME_IDX 3
#define PARAM_ACT_CBPARAMNAME_IDX 4

static int get_prefixed_tbl_keyfield(struct p4tc_filter_ctx *ctx,
				     struct p4tc_table_keyfield *keyfield,
				     const char **p4tcpath)
{
	const char *keyname = p4tcpath[PARAM_KEY_PARAM_NAME_IDX];
	struct p4tc_json_key_fields_list *key;
	struct p4tc_json_table *t = ctx->t;

	if (!t) {
		err_type("Table introspection data missing");
		return -1;
	}

	key = p4tc_json_find_table_keyfield(t, 1, keyname);
	if (!key) {
		err_type_fmt("Unable to find key field %s in intros file\n",
			     keyname);
		return -1;
	}
	...
	if (!keyfield->key_type) {
		err_type_fmt("Unable to find type %s\n", key->type);
		return -1;
	}
...
static int get_prefixed_act_param_type(struct p4tc_filter_ctx *ctx,
				       ...
				       const char **p4tcpath)
{
	...
	if (p4tcpath[PARAM_ACT_CBPARAMNAME_IDX]) {
		const char *actname;
		const char *cbname;

		cbname = p4tcpath[PARAM_ACT_CBNAME_IDX];
		actname = p4tcpath[PARAM_ACT_CBACTNAME_IDX];

		ret = concat_cb_name(full_actname, cbname, actname, ACTNAMSIZ);
		if (ret < 0) {
			err_type("Action name too long");
			return -1;
		}
		snprintf(full_actname, ACTNAMSIZ, "%s/%s", cbname, actname);
		paramname = p4tcpath[PARAM_ACT_CBPARAMNAME_IDX];
	} else {
		try_strncpy(full_actname, p4tcpath[PARAM_ACT_ACTNAME_IDX],
			    ACTNAMSIZ);
		paramname = p4tcpath[PARAM_ACT_PARAMNAME_IDX];
	}

	act = p4tc_json_find_act(ctx->p, full_actname);
	if (!act) {
		err_type_fmt("Unable to find action %s in intros",
			     full_actname);
		return -1;
	}

	if (ctx->t) {
		if (!p4tc_json_find_table_act(ctx->t, full_actname)) {
			err_type_fmt("Action does not belong to table %s",
				     ctx->t->name);
			return -1;
		}
	}

	ret = p4tc_act_param_build(act, param, paramname, true);
			     ...
			     char **p4tcpath, int num_components)
{
	...
	ret = get_prefixed_act_param_type(ctx, param,
					  (const char **)p4tcpath);
	if (ret < 0)
		goto free_param;
	...
	if (num_components == 5)
		ret = snprintf(filter_name_expr->act_param.fullactname,
			       ACTNAMSIZ, "%s/%s/%s", ctx->p->name,
			       p4tcpath[PARAM_ACT_CBNAME_IDX],
			       p4tcpath[PARAM_ACT_CBACTNAME_IDX]);
	else
		ret = snprintf(filter_name_expr->act_param.fullactname,
			       ACTNAMSIZ, "%s/%s", ctx->p->name,
			       p4tcpath[PARAM_ACT_ACTNAME_IDX]);

	if (ret >= ACTNAMSIZ) {
		fprintf(stderr, "full actname too long\n");
		goto free_param;
	}
 ...
 * - key.keyfield
 */
static int parse_prefixed_exp(struct p4tc_filter_ctx *ctx, struct typedexpr *t,
			      struct parsedexpr *e, const char *prefix_exp)
{
	union p4tc_filter_name_expr *filter_name_expr;
	...
	if (strcmp(p4tcpath[PARAM_IDX], "param") == 0) {
		...
		if (strcmp(p4tcpath[PARAM_OBJ_IDX], "act") == 0) {
			struct p4tc_act_param *param;

			if (ctx->obj_id != P4TC_OBJ_RUNTIME_TABLE) {
				err_type("Context isn't of table type");
				return -EINVAL;
			}

			param = parse_prefixed_act_param_exp(ctx, t,
							     filter_name_expr,
							     e, p4tcpath,
							     num_components);
			if (!param)
		...
		t->name.name = e->name.name;
		t->name.data = filter_name_expr;
	} else if (strcmp(p4tcpath[PARAM_IDX], "key") == 0) {
		struct p4tc_table_keyfield *keyfield;
		...
		ret = get_prefixed_tbl_keyfield(ctx, keyfield,
						(const char **)p4tcpath);
		if (ret < 0) {
			free(keyfield);
			free(filter_name_expr);
...
static struct typedexpr *
type_prefixed_name_exp(struct p4tc_filter_ctx *ctx, struct parsedexpr *e)
{
	...
	ret = parse_prefixed_exp(ctx, typed_prefix, e, prefixed_exp);
	if (ret < 0) {
		free(typed_prefix);
		return &typed_error;
	}
...
static bool expr_is_cmd(struct parsedexpr *e)
{
	return !strcmp(e->name.name, "cmd");
}
					  ...
					  __u32 uapi_attr)
{
	...
	struct known_unprefixed_name *ret_unprefixed_name;
	...
	ret_unprefixed_name = lookup_known_unprefixed_names(name);
	if (ret_unprefixed_name) {
		fprintf(stderr, "Unprefixed name %s already registered \n",
			name);
		return -1;
	}
...
int register_known_unprefixed_names(void)
{
	...
	if (ret < 0) {
		fprintf(stderr, "Unable to unprefixed name msecs_since\n");
		return ret;
	}

	ret = register_known_unprefixed_name("cmd", P4TC_T_U32,
					     P4TC_FILTER_OPND_CMD);
	if (ret < 0) {
		fprintf(stderr, "Unable to unprefixed name cmd\n");
		return ret;
	}
					      ...
					      struct p4_type_s *prefix_name_type)
{
	...
	case P4TC_T_RATE: {
		...
		if (!typed_constant)
			return err_type("Out of memory");
		val.value = &typed_constant->rate.rate;
		val.bitsz = rate_type->bitsz;
		ret = rate_type->parse_p4t(&val,
					  (const char *)string->string.txt,
					  0);
		if (ret < 0)
			free(typed_constant);

		typed_constant->t = ET_RATE;
...
static struct typedexpr *type_command_rhs(struct parsedexpr *rhs)
{
	struct typedexpr *t;
	...
	t->t = ET_INTEGER;
	t->integer.typ = get_p4type_byid(P4TC_T_U32);

	if (!strcmp(rhs->name.name, "create"))
		t->integer.i = RTM_P4TC_CREATE;
	else if (!strcmp(rhs->name.name, "update"))
		t->integer.i = RTM_P4TC_UPDATE;
	else if (!strcmp(rhs->name.name, "delete"))
		t->integer.i = RTM_P4TC_DEL;
	else {
		free(t);
		return err_type("Invalid command");
	}
 ...
 */
static struct typedexpr *
type_binary_relational(struct p4tc_filter_ctx *ctx, struct parsedexpr *e)
{
	...
	if (lhs->t == ET_NAME) {
		if (strchr(lhs->name.name, '.')) {
			typed_lhs = type_prefixed_name_exp(ctx, lhs);
	...
	if (typed_lhs) {
		...
		if (expr_is_cmd(lhs)) {
			typed_rhs = type_command_rhs(rhs);
...
free_typed_ops:
	if (typed_rhs && typed_rhs->t != ET_ERR)
		free_typedexpr(typed_rhs);
	if (typed_lhs && typed_lhs->t != ET_ERR)
		free_typedexpr(typed_lhs);

	return &typed_error;
}

static struct typedexpr *type_unary(struct p4tc_filter_ctx *ctx,
				    struct parsedexpr *e)
{
	...
	case U_MINUS:
		break;
	case U_LOGNOT:
		typed_arg = type_expr(ctx, e->unary.arg);
		break;
	default:
		break;
	}
...
static struct typedexpr *type_binary(struct p4tc_filter_ctx *ctx,
				     struct parsedexpr *e)
{
	struct typedexpr *typed_binary;
	...
	case B_LE:
	case B_GE:
		return type_binary_relational(ctx, e);
	case B_LOGAND:
	case B_LOGOR:
	case B_LOGXOR:
	default:
	...
	typed_lhs = type_expr(ctx, e->binary.lhs);
	if (typed_lhs->t == ET_ERR)
		return typed_lhs;
	typed_rhs = type_expr(ctx, e->binary.rhs);
	if (typed_rhs->t == ET_ERR)
		return typed_rhs;
...
struct typedexpr *type_expr(struct p4tc_filter_ctx *ctx, struct parsedexpr *e)
{
	switch (e->t) {
	case ET_BINARY:
		return type_binary(ctx, e);
	case ET_UNARY:
		return type_unary(ctx, e);
	default:
		/* Will never happen */
...
static void
extract_relational_val(void *value, void *mask, struct typedexpr *val,
		       bool *extracted_mask)
{
	switch (val->t) {
	case ET_BOOL: {
				...
				struct typedexpr *val, __u32 rel_attr)
{
	...
	case P4TC_NAME_EXPR_CUSTOM: {
		...
		case P4TC_FILTER_OPND_CMD: {
			__u32 value;

			extract_relational_val(&value, NULL, val,
					       &extracted_mask);
			addattr32(n, MAX_MSG, known_unprefixed_name->uapi_attr,
				  value);
			break;
...
static void
print_filter_custom_name_expr_type(struct known_unprefixed_name *known_unprefixed_name)
{
	printf("%s\n", known_unprefixed_name->name);
}
...
print_filter_name_expr_type(union p4tc_filter_name_expr *filter_name_expr)
{
	...
	case P4TC_NAME_EXPR_CUSTOM:
		print_filter_custom_name_expr_type(filter_name_expr->custom.known_unprefixed_name);
		break;
	default:
		/* Should never happen */
		break;
...
void dump_typed_expr(struct typedexpr *t, int indent)
{
	...
	case ET_RATE: {
		struct p4_type_s *rate_type =
			get_p4type_byid(P4TC_T_RATE);
		struct p4_type_value val = {0};

		val.value = &t->rate.rate;
		rate_type->print_p4t("rate", NULL, &val, stdout);
		printf("\n");
		break;
	}
	case ET_NAME:
