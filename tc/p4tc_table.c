/*
 * p4tc_table.c		P4 TC Table Management
 *
...
{
	__u8 buffer_bytesz = BITS_TO_BYTES(buffer_bitsz);
	__u8 byte_per_type = BITS_TO_BYTES(type->bitsz);
	__u8 index = 0;

#if __BYTE_ORDER == __LITTLE_ENDIAN
	if (p4type_bigendian(type))
#endif
		index = byte_per_type - buffer_bytesz;

	return index;
}

static __u8 *get_value_ptr(struct p4_type_value *val,
			   struct p4_type_s *type)
{
	__u8 index = get_buffer_index(val->value, val->bitsz, type);
	__u8 *value_u8 = val->value;

	return &value_u8[index];
}

static __u8 *get_mask_ptr(struct p4_type_value *val,
			   struct p4_type_s *type)
{
	__u8 index = get_buffer_index(val->mask, val->bitsz, type);
	__u8 *mask_u8 = val->mask;

	return &mask_u8[index];
}

static void parse_common(__u8 *keyblob, __u8 *maskblob,
			 struct p4_type_value *val, __u32 *offset,
			 struct p4_type_s *type)
{
	__u8 *value_u8 = get_value_ptr(val, type);
	__u8 *mask_u8 = get_mask_ptr(val, type);
	size_t sz = BITS_TO_BYTES(val->bitsz);

	memcpy((keyblob + *offset), value_u8, sz);
	memcpy((maskblob + *offset), mask_u8, sz);
	*offset += sz;
}
		      ...
		      struct p4tc_json_table *table, __u32 tbl_id)
{
	...
	if (len != RTA_PAYLOAD(tb[P4TC_ENTRY_MASK_BLOB]) ||
	    (table && len != BITS_TO_BYTES(table->ksize))) {
		if (table)
			fprintf(stderr,
			 ...
			 struct rtattr *arg, FILE *f)
{
	...
	if (cmd == RTM_P4TC_DEL && (n->nlmsg_flags & NLM_F_ROOT))
		...
		if (tb[P4TC_PARAMS])
			print_table_entry(n, tb[P4TC_PARAMS], f, "", pipe,
					  table, tbl_id ? *tbl_id : 0);
	}
...
static struct p4tc_json_table *
introspect_table_byname(struct p4tc_json_pipeline **p,
			const char *pname, const char **p4tcpath)
{
	const char *tblname = p4tcpath[PATH_TBLNAME_IDX];
	const char *cbname = p4tcpath[PATH_CBNAME_IDX];
	char full_tblname[P4TC_TABLE_NAMSIZ] = {0};
			     ...
			     const char **p4tcpath, const char *keyname)
{
	...
	*t = introspect_table_byname(p, pname, p4tcpath);
	if (!(*t))
		return NULL;
			    ...
			    char **p4tcpath, const char *pname, __u32 tbl_id)
{
	...
	if (state->num_keys + 1 != key->order) {
		fprintf(stderr,
			"Error: Key component \"%s\" should be component #%d and not #%d\n",
			*argv, key->order, state->num_keys + 1);
		return -1;
	}
...
static int parse_table_entry_filter(int *argc_p, char ***argv_p,
				    ...
				    __u32 tbl_id)
{
	char *pname = p4tcpath[PATH_RUNTIME_PNAME_IDX];
	struct p4tc_filter_ctx ctx = {
		.obj_id = P4TC_OBJ_RUNTIME_TABLE,
	};
	...
	t = introspect_table_byname(&ctx.p, pname, (const char **)p4tcpath);
	if (!t)
		return -1;
	ctx.t = t;
