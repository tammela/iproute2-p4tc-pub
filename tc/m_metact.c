...
int add_commands(struct nlmsghdr *n, int ins_cnt, int tca_id)
{
	...
	tailinsl = addattr_nest(n, MAX_MSG, tca_id);
	for (i = 0; i < ins_cnt; i++) {
		tailins = addattr_nest(n, MAX_MSG, i+1);
		...
		if (ins->Apresent) {
			tailoperA = addattr_nest(n, MAX_MSG, TCAA_METACT_OPER_A);
		...
		if (ins->Bpresent) {
			tailoperB = addattr_nest(n, MAX_MSG, TCAA_METACT_OPER_B);
			if (ins->pathB) {
		...
		if (ins->Cpresent) {
			tailoperC = addattr_nest(n, MAX_MSG, TCAA_METACT_OPER_C);
			if (ins->pathC) {
			...
			int tca_id, struct nlmsghdr *n)
{
	...
	tailma = addattr_nest(n, MAX_MSG, tca_id);

	addattr_l(n, MAX_MSG, TCA_METACT_PARMS, &p, sizeof(p));

	if (ins_cnt)
		tailinsl = addattr_nest(n, MAX_MSG, TCA_METACT_LIST);
	else
